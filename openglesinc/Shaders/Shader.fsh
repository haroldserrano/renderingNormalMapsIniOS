...
//1a. declare a uniform sampler 2d that contains the normal texture
...
mediump vec4 SpecularMaterialColor=vec4(0.5,0.5,0.5,1.0);
...
//37. compute the light direction vector, illuminance and attenuation
computePointLightValues(positionInViewSpace);

//38. Sample the Normal Texture Map
mediump vec4 normalTexture=texture2D(NormalTextureMap,vTexCoordinates.st);
...
//41. Mix the texture color and fragmentColor
mediump vec4 finalMixedColor=mix(textureColor,finalLightColor,0.5);

//42. set the final color to the output of the fragment shader
